...
import { Search, MessageSquare, Loader2, Send, Clock, AlertCircle, User } from 'lucide-react';
...
export default function ProviderMessages() {
  ...
  const filteredConversations = conversations.filter(c => {
    ...
    return customerName.toLowerCase().includes(query) || category.toLowerCase().includes(query);
  });

  return (
    <div className="flex flex-col h-[calc(100vh-140px)] rounded-[2rem] border border-white/5 bg-[#0f172a]/60 backdrop-blur-xl overflow-hidden shadow-2xl relative animate-fadeInUp">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {loading ? (
              ...
              <div className="divide-y divide-white/5">
                {filteredConversations.map(conv => {
                  ...
                  return (
                    ...
                    >
                      <div className="h-10 w-10 shrink-0 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center font-bold text-white shadow-inner relative">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-0.5">
                          <h4 className="text-sm font-bold text-slate-200 truncate pr-2">{customerName}</h4>
                        ...
                        <p className="text-[11px] font-medium text-emerald-400 truncate mb-1">{conv.serviceCategory}</p>
              ...
              <div className="flex flex-col items-center justify-center h-full p-6 text-center">
                ...
                <p className="text-sm font-bold text-slate-300 mb-1">No messages</p>
                <p className="text-xs text-slate-500">Your customer conversations will appear here.</p>
              </div>
            )}
          </div>
        </div>
          ...
          {activeConversation ? (
            <>
              ...
              <div className="h-20 border-b border-white/5 bg-[#0f172a]/90 backdrop-blur-md px-6 flex items-center gap-4 shrink-0">
                ...
                <div className="h-10 w-10 shrink-0 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center font-bold text-white shadow-inner">
                  {(activeConversation.customer?.name || 'C').charAt(0)}
                </div>
                <div>
                  <h3 className="text-base font-bold text-white">{activeConversation.customer?.name}</h3>
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-emerald-400">{activeConversation.serviceCategory}</span>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 hidden lg:flex">
              <div className="h-20 w-20 rounded-[2rem] bg-emerald-500/5 border border-emerald-500/10 flex items-center justify-center mb-6 shadow-inner">
                <MessageSquare className="h-8 w-8 text-emerald-400" />
              </div>
              <h3 className="text-2xl font-bold text-white tracking-tight mb-2">Customer Messages</h3>
              <p className="text-slate-500 text-sm max-w-sm">Select a conversation from the sidebar to communicate with your customers.</p>
            </div>
          )}
        </div>
